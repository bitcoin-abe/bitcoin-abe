...
from Abe.util import decode_address, hex2b
from . import data
        ...
        _gen.block(
            ...
            transactions=[
                ...
                _gen.tx(
                    txIn=[
                        _gen.txin(
                            prevout=blocks[1]["transactions"][0]["txOut"][0],
                            scriptSig="XXX",
                        )
                    ],
                    txOut=[
...
def test_an3j4_subbinaddr(an3j4):
...
def test_b14_t1i0_binaddr(b14, gen):
    assert b14["transactions"][1]["in"][0]["binaddr"] == gen.chain.pubkey_hash(
        data.PUBKEYS[1]
    )
...
def test_b14_t1o2k0(b14, gen):
    assert b14["transactions"][1]["out"][2]["subbinaddr"][0] == gen.chain.pubkey_hash(
        data.PUBKEYS[2]
    )


def test_b14_t1o2k1(b14, gen):
    assert b14["transactions"][1]["out"][2]["subbinaddr"][1] == gen.chain.pubkey_hash(
        data.PUBKEYS[3]
    )


def test_b14_t1o2k2(b14, gen):
    assert b14["transactions"][1]["out"][2]["subbinaddr"][2] == gen.chain.pubkey_hash(
        data.PUBKEYS[4]
    )
...
def test_b14t1o2_subbinaddr(b14t1, gen):
    assert b14t1["out"][2]["subbinaddr"] == [
        gen.chain.pubkey_hash(pubkey) for pubkey in data.PUBKEYS[2:5]
    ]
