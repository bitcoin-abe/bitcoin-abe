...
def run_upgrades_locked(store, _upgrades):
    for i in range(len(_upgrades) - 1):
        vers, func = _upgrades[i]
        if store.config["schema_version"] == vers:
            schema_version = _upgrades[i + 1][0]
            store.log.warning("Upgrading schema to version: %s", schema_version)
            func(store)
            if schema_version[:3] == "Abe":
                store.sql(
...
def run_upgrades(store, _upgrades):
    """Guard against concurrent upgrades."""
    lock = store.get_lock()
    try:
        run_upgrades_locked(store, _upgrades)
    finally:
...
def index_cc_block(store):
    store.sql("""CREATE INDEX x_cc_block ON chain_candidate (block_id)""")
...
def init_block_value_in(store):
    ...
    """
        ...
        store.sql(
            "UPDATE block SET block_value_in = ? WHERE block_id = ?",
...
def init_block_value_out(store):
    ...
    """
        ...
        store.sql(
            "UPDATE block SET block_value_out = ? WHERE block_id = ?",
            (int(row[1]), row[0]),
        )
...
def init_block_totals(store):
    ...
    for row in store.selectall(
        ...
        if prev_id is None:
            stats[block_id] = {"chain_start": nTime, "satoshis": generated}
        else:
            stats[block_id] = {
        ...
        store.sql(
            "UPDATE block SET block_total_seconds = ?,"
...
def init_satoshi_seconds_destroyed(store):
    ...
    while start <= stop:
        ...
        for row in cur:
            ...
            count += 1
            store.sql(
                "UPDATE block_tx SET satoshi_seconds_destroyed = ?"
...
def init_block_satoshi_seconds(store):
    ...
    while True:
        ...
        if prev_id is None:
            ...
            created = stats[prev_id]["satoshis"] * (nTime - stats[prev_id]["nTime"])
            stats[block_id] = {
                "satoshis": satoshis,
...
def drop_block_ss_columns(store):
    ...
    for column in ["created", "destroyed"]:
        try:
            store.sql("ALTER TABLE block DROP COLUMN block_ss_" + column)
        except Exception:
            store.rollback()
...
def add_constraint(store, table, name, constraint):
    try:
        store.sql("ALTER TABLE " + table + " ADD CONSTRAINT " + name + " " + constraint)
    except Exception:
        store.log.exception(
            "Failed to create constraint on table "
            + table
            + ": "
            + constraint
            + "; ignoring error."
...
def add_chk_block_txin_out_block_id_nn(store):
    add_constraint(
        store, "block_txin", "chk3_block_txin", "CHECK (out_block_id IS NOT NULL)"
    )
...
def create_x_cc_block_height(store):
    store.sql("CREATE INDEX x_cc_block_height ON chain_candidate (block_height)")
...
def populate_abe_sequences(store):
    if store.config["sql.sequence_type"] == "update":
        ...
        for transaction in [
            ...
            (last_id,) = store.selectrow(
                f"SELECT MAX({transaction}_id) FROM {transaction}"
            )
            if last_id is None:
                continue
            store.sql(
                "UPDATE abe_sequences SET nextid = ? WHERE key = ?" " AND nextid <= ?",
                (last_id + 1, transaction, last_id),
            )
            if store.rowcount() < 1:
                store.sql(
                    "INSERT INTO abe_sequences (key, nextid)" " VALUES (?, ?)",
...
def repair_missed_blocks(store):
    ...
    for row in best_work:
        ...
        if in_longest == 1:
            store.log.info("Chain %d already has the block of greatest work.", chain_id)
            continue
    ...
    for elt in best:
        ...
        while True:
            ...
            if store.rowcount() != 1:
                raise Exception(
                    "Wrong rowcount updating chain_candidate ("
                    + str(chain_id)
                    + ", "
                    + str(block_id)
                    + ")"
...
def rename_abe_sequences_key(store):
    ...
    for row in data:
        store.sql(
            "INSERT INTO abe_sequences (sequence_key, nextid)" " VALUES (?, ?)", row
        )
...
def insert_null_pubkey(store):
    dbnull = store.binin(NULL_PUBKEY_HASH)
    row = store.selectrow(
        "SELECT pubkey_id FROM pubkey WHERE pubkey_hash = ?", (dbnull,)
    )
    if row:
        ...
        temp_hash = "".join([chr(randint(0, 255)) for x in range(20)])
        store.sql(
            "INSERT INTO pubkey (pubkey_id, pubkey_hash) VALUES (?, ?)",
            (NULL_PUBKEY_ID, store.binin(temp_hash)),
        )
        store.sql(
            "UPDATE txout SET pubkey_id = ? WHERE pubkey_id = ?",
            (NULL_PUBKEY_ID, old_id),
        )
        store.sql("DELETE FROM pubkey WHERE pubkey_id = ?", (old_id,))
        store.sql(
            "UPDATE pubkey SET pubkey_hash = ? WHERE pubkey_id = ?",
            (dbnull, NULL_PUBKEY_ID),
        )
    else:
        store.sql(
            """
            INSERT INTO pubkey (pubkey_id, pubkey_hash) VALUES (?, ?)""",
            (NULL_PUBKEY_ID, dbnull),
        )
...
def set_netfee_pubkey_id(store):
    ...
    store.sql(
        ...
        (NULL_PUBKEY_ID, store.binin(SCRIPT_NETWORK_FEE)),
    )
...
def adjust_block_total_satoshis(store):
    ...
    for block_id in block_ids:
        blck = block[block_id]
        prev_id = blck["prev_id"]
        prev_lost = 0 if prev_id is None else block[prev_id]["cum_lost"]
        blck["cum_lost"] = blck.get("lost", 0) + prev_lost
...
def populate_search_block_id(store):
    ...
    for block_id, height, prev_id in store.selectall(
        ...
        if prev_id is not None:
            ...
            store.sql(
                "UPDATE block SET search_block_id = ? WHERE block_id = ?",
                (search_id, block_id),
            )
...
def populate_chain_magic(store):
    for chain_id, magic in store.selectall(
        ...
        store.sql(
            "UPDATE chain SET chain_magic = ? WHERE chain_id = ?", (magic, chain_id)
        )
...
def populate_chain_script_addr_vers(store):
    def update(addr_vers, script_vers):
        store.sql(
            "UPDATE chain SET chain_script_addr_vers=? WHERE chain_address_version=?",
...
def populate_multisig_pubkey(store):
    ...
    for txout_id, chain_id, db_script in rows:
        ...
        if pubkey_id > 0:
            store.sql(
                "UPDATE txout SET pubkey_id = ? WHERE txout_id = ?",
                (pubkey_id, txout_id),
            )
            count += 1
    store.commit()
...
def upgrade_schema(store):
    ...
    curr = upgrades[-1][0]
    if schema_version != curr:
        raise Exception(
            "Can not upgrade from schema version %s to %s\n" % (schema_version, curr)
        )
