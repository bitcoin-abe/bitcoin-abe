...
class AbeVerify:
    ...
    def verify_blockchain(self, chain_id) -> None:
        ...
        for block_id, block_height in self.store.selectall(
            ...
            + """
          ORDER BY block_height ASC, block_id ASC""",
            params,
        ):
    ...
    def verify_tx_merkle_hash(self, block_id) -> None:
        ...
        if len(tree) != num_tx:
            self.logger.info(
                "block %d (id %d): block_num_tx=%d but found %d",
                block_height,
                block_id,
                num_tx,
                len(tree),
            ...
            for (txin_id,) in self.store.selectall(
              ...
              ORDER BY txin.txin_id ASC""",
                ...
                if txin_id not in known_ids:
                    missing.add(txin_id)
                    self.logger.info(
                        "block id %d: txin_id %d not found in " "block_txin",
                        block_id,
                        txin_id,
                    )

            # Check all txin_id's already present (what we would insert)
            for txin_id, obt_id in self._populate_block_txin(
                int(block_id), skip_txin=missing, check_only=True
            ):
                if obt_id != known_ids[txin_id]:
                    redo.add(txin_id)
                    self.logger.info(
                        "block id %d: txin_id %d out_block_id " "is %d (should be %s)",
                        block_id,
                        txin_id,
                        known_ids[txin_id],
                        obt_id,
    ...
    def verify_block_stats(self, block_id, chain_id):
        ...
        if None in (prev_satoshis, prev_seconds, prev_ss, prev_total_ss):
            if block_height == 0:
                ...
                self.logger.info(
                    "block %d (id %d): Bad prev block, skipping "
                    "as assumed bad block",
                    block_height,
                    block_id,
                )
        ...
        b["total_satoshis"] = (
            prev_satoshis + b["value_out"] - b["value_in"] - value_destroyed
        )
        ...
        for key in self.blkstats:
            if d[key] != b[key]:
                badcheck = True
                self.logger.info(
                    "block %d (id %d): %s do not match: %s " "(should be %s)",
                    block_height,
                    block_id,
                    key,
                    d[key],
        ...
        if badcheck and self.repair:
            ...
            self.logger.info("block %d (id %d): stats repaired", block_height, block_id)

        if badcheck:
            self.stats["sbad"] += 1

    # Copied and modified from the same function in data_store.py
    def _populate_block_txin(self, block_id, skip_txin=None, check_only=False):
...
"""
    ...
    for (chain_id,) in store.selectall(
        ...
        logger.warning(
            "Checking %s chain (id %d) at height %d",
            chain.name,
            chain_id,
            (chk.block_min if chk.block_min else 0),
        )
