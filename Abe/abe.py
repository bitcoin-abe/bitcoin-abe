...
__version__ = importlib.metadata.version("Abe")
...
class Abe:
    def __init__(self, store: DataStore, args):
        ...
        self.static_path = "" if args.static_path is None else args.static_path
        self.template_vars = args.template_vars.copy()
        self.template_vars["STATIC_PATH"] = self.template_vars.get(
            "STATIC_PATH", self.static_path
        )
        self.template = flatten(args.template)
        self.debug = args.debug
        self.log = logging.getLogger(__name__)
        self.log.info("Abe initialized.")
        self.home = str(self.template_vars.get("HOMEPAGE", DEFAULT_HOMEPAGE))
        if not args.auto_agpl:
            self.template_vars["download"] = self.template_vars.get("download", "")
        self.base_url = args.base_url
        self.address_history_rows_max = int(args.address_history_rows_max or 1000)

        if args.shortlink_type is None:
            self.shortlink_type = (
                "firstbits" if store.use_firstbits else "non-firstbits"
            )
        else:
    ...
    def __call__(self, env, start_response):
        ...
        page = {
            ...
            "content_type": str(self.template_vars["CONTENT_TYPE"]),
        ...
        if self.fix_path_info(env):
            self.log.debug("fixed path_info")
        ...
        try:
            if handler is None:
                return self.serve_static(cmd + env["PATH_INFO"], start_response)

            if not self.args.no_load:
                # Always be up-to-date, even if we mean having to wait
            ...
            page["body"] += [
                '<p class="error"> Sorry, I don\'t know about that chain!</p>\n'
            ]
        except Redirect:
            return redirect(page)
        except Streamed:
        ...
        self.store.rollback()  # Close implicitly opened transaction.

        start_response(
            page["status"],
            [("Content-type", page["content_type"]), ("Cache-Control", "max-age=30")],
        )

        tvars["title"] = flatten(page["title"])
        tvars["h1"] = flatten(page.get("h1") or page["title"])
        tvars["body"] = flatten(page["body"])
        if self.args.auto_agpl:
            tvars["download"] = ' <a href="' + page["dotdot"] + 'download">Source</a>'
    ...
    def get_handler(self, cmd):
        return getattr(self, "handle_" + cmd, None)

    def handle_chains(self, page):
        page["title"] = ABE_APPNAME + " Search"
        body = page["body"]
        body += [
            self.search_form(page),
            "<table>\n",
        ...
        for row in rows:
            name = row[0]
            chain = self.store.get_chain_by_name(name)
            if chain is None:
                self.log.warning("Store does not know chain: %s", name)
                continue
            ...
            if row[1] is not None:
                ...
                if row[6] is not None and row[7] is not None:
                    (seconds, satoshis, sat_seconds, total_ss) = (
                        int(row[4]),
                        int(row[5]),
                        int(row[6]),
                        int(row[7]),
                    ...
                    started = n_time - seconds
                    chain_age = now - started
                    since_block = now - n_time

                    if satoshis == 0:
                        avg_age = "&nbsp;"
                    else:
                        avg_age = f"{((float(sat_seconds) / satoshis + since_block) / 86400.0)}"
                    ...
                    if chain_age <= 0:
                        ...
                        if denominator <= 0:
                            ...
                            percent_destroyed = f"{100.0 - (100.0 * (sat_seconds + more) / denominator)}"

                    body += [
                        "<td>",
                        format_time(started)[:10],
                        "</td>",
                        "<td>",
                        f"{chain_age / 86400.0}",
                        "</td>",
    ...
    def chain_lookup_by_name(self, symbol):
        if symbol is None:
            ret = self.get_default_chain()
        else:
            ret = self.store.get_chain_by_name(symbol)
        if ret is None:
            raise NoSuchChainError()
        return ret

    def get_default_chain(self):
        return self.store.get_default_chain()

    def format_addresses(self, data, dotdot, chain):
        if data["binaddr"] is None:
            return "Unknown"
        if "subbinaddr" in data:
            # Multisig or known P2SH.
            ret = [
                hash_to_address_link(
                    chain.script_addr_vers, data["binaddr"], dotdot, text="Escrow"
                ),
                " ",
            ...
            for binaddr in data["subbinaddr"]:
                ret += [
                    " ",
                    hash_to_address_link(data["address_version"], binaddr, dotdot, 10),
                ]
            return ret
        return hash_to_address_link(data["address_version"], data["binaddr"], dotdot)

    def call_handler(self, page, cmd):
        handler = self.get_handler(cmd)
        if handler is None:
            raise PageNotFound()
        handler(page)

    def handle_chain(self, page):
        symbol = wsgiref.util.shift_path_info(page["env"])
        chain = self.chain_lookup_by_name(symbol)
        page["chain"] = chain
        ...
        if cmd == "chain" or cmd == "chains":
            raise PageNotFound()
        if cmd is not None:
            self.call_handler(page, cmd)
            return

        page["title"] = chain.name

        body = page["body"]
        body += self.search_form(page)
        ...
        try:
            height = get_int_param(page, "hi")
        except ValueError:
            height = None
        orig_hi = height

        if height is None:
            row = self.store.selectrow(
                """
        ...
        if height is None:
            if orig_hi is None and count > 0:
                ...
                body += [
                    '<p class="error">The requested range contains no blocks.</p>\n'
                ]
            return
        ...
        """,
            (chain.id, height - count + 1, height, count),
        )
        ...
        nav = ['<a href="', basename, "?count=", str(count), '">&lt;&lt;</a>']
        nav += [
            ' <a href="',
            basename,
            "?hi=",
        ...
        for count_iter in (20, 50, 100, 500, 2016):
            nav += [" "]
            if count_iter != count:
                nav += ['<a href="', basename, "?count=", str(count_iter)]
                if height is not None:
                    nav += ["&amp;hi=", str(max(height, count_iter - 1))]
                nav += ['">']
            nav += [" ", str(count_iter)]
            if count_iter != count:
                nav += ["</a>"]
        ...
        body += [
            ...
            '<a href="https://en.bitcoin.it/wiki/Bitcoin_Days_Destroyed">',
            "CoinDD</a></th>",
            ["<th>Satoshi-seconds</th>", "<th>Total ss</th>"] if extra else "",
            "</tr>\n",
        ]
        for row in rows:
            (
                ...
                sat_seconds,
                satoshis,
                destroyed,  # pylint: disable=unused-variable
                total_ss,
            ) = row
            n_time = int(n_time)
            ...
            sat_seconds = int(sat_seconds) if sat_seconds else 0
            total_ss = int(total_ss) if total_ss else 0

            if satoshis == 0:
                avg_age = "&nbsp;"
            else:
                avg_age = f"{sat_seconds / satoshis / 86400.0}"

            if total_ss <= 0:
                percent_destroyed = "&nbsp;"
            else:
                percent_destroyed = f"{100.0 - (100.0 * sat_seconds / total_ss)}"

            body += [
                '<tr><td><a href="',
                page["dotdot"],
                "block/",
                self.store.hashout_hex(_hash),
                '">',
                height,
                "</a></td><td>",
                format_time(int(n_time)),
                "</td><td>",
                num_tx,
                "</td><td>",
                format_satoshis(value_out, chain),
                "</td><td>",
                util.calculate_difficulty(int(n_bits)),
                "</td><td>",
                format_satoshis(satoshis, chain),
                "</td><td>",
                avg_age,
                ...
                "</td><td>",
                percent_destroyed,
                ["</td><td>", f"{sat_seconds}", "</td><td>", f"{total_ss}"]
                if extra
                else "",
                "</td></tr>\n",
            ]
    ...
    def _show_block(self, page, dotdotblock, chain, **kwargs):
        body = page["body"]

        try:
            block = self.store.export_block(chain, **kwargs)
        except data_store.MalformedHash:
        ...
        in_longest = False
        for cc in block["chain_candidates"]:
            if chain is None:
                chain = cc["chain"]
        ...
        if in_longest:
            ...
            page["title"] = ["Block ", block["hash"][:4], "...", block["hash"][-10:]]

        body += self.short_link(page, "b/" + block_shortlink(block["hash"]))

        is_stake_chain = chain.has_feature("nvc_proof_of_stake")
        is_stake_block = is_stake_chain and block["is_proof_of_stake"]
        ...
        if is_stake_chain:
            body += [
                ...
                ": ",
                format_satoshis(block["generated"], chain),
                " coins generated<br />\n",
            ]
        body += ["Hash: ", block["hash"], "<br />\n"]

        if block["hashPrev"] is not None:
            body += [
                'Previous Block: <a href="',
                dotdotblock,
                block["hashPrev"],
                '">',
                block["hashPrev"],
        ...
        if block["next_block_hashes"]:
            body += ["Next Block: "]
        for _hash in block["next_block_hashes"]:
            body += ['<a href="', dotdotblock, _hash, '">', _hash, "</a><br />\n"]
        ...
        body += [
            ...
            block["nTime"],
            " (",
            format_time(block["nTime"]),
            ")<br />\n",
            "Difficulty: ",
            format_difficulty(util.calculate_difficulty(block["nBits"])),
            f" (Bits: {block['nBits']})",
            "<br />\n",
            [
                "Cumulative Difficulty: ",
                format_difficulty(util.work_to_difficulty(block["chain_work"])),
                "<br />\n",
            ]
            if block["chain_work"] is not None
            else "",
            "Nonce: ",
            block["nNonce"],
            "<br />\n",
            ...
            "<br />\n",
            "Value out: ",
            format_satoshis(block["value_out"], chain),
            "<br />\n",
            "Transaction Fees: ",
            format_satoshis(block["fees"], chain),
            "<br />\n",
            [
                f"Average Coin Age: {block['satoshi_seconds'] / 86400.0 / block['chain_satoshis']}",
                " days<br />\n",
            ]
            if block["chain_satoshis"] and (block["satoshi_seconds"] is not None)
            else "",
            "</p>\n",
        ]
        ...
        body += [
            "<table><tr><th>Transaction</th><th>Fee</th>"
        ...
        for tx in block["transactions"]:
            ...
            if tx is block["transactions"][0]:
                ...
                for txin in tx["in"]:
                    body += [
                        self.format_addresses(txin, page["dotdot"], chain),
                        ": ",
            ...
            for txout in tx["out"]:
                if is_stake_block:
                    if tx is block["transactions"][0]:
                        ...
                        assert len(tx["out"]) == 1
                        body += [
                            format_satoshis(block["proof_of_stake_generated"], chain),
                ...
                body += [
                    self.format_addresses(txout, page["dotdot"], chain),
                    ": ",
                    format_satoshis(txout["value"], chain),
                    "<br />",
                ]
    ...
    def handle_block(self, page):
        ...
        self._show_block(page, "", None, block_hash=block_hash)

    def handle_tx(self, page):
        ...
        try:
            ...
            tx = self.store.export_tx(tx_hash=tx_hash, format="browser")
        except data_store.MalformedHash:
            body += ['<p class="error">Not in correct format.</p>']
            return
        ...
        return self.show_tx(page, tx)

    def show_tx(self, page, tx):
        body = page["body"]
        ...
        def row_to_html(row, this_ch, other_ch, no_link_text):
            ...
            body += [
                "</td>\n",
                "<td>",
                format_satoshis(row["value"], chain),
                "</td>\n",
                "<td>",
                self.format_addresses(row, "../", chain),
                "</td>\n",
            ]
        ...
        body += self.short_link(page, "t/" + hexb58(tx["hash"][:14]))
        ...
        for tx_cc in tx["chain_candidates"]:
            ...
            body += [
                ...
                tx_cc["block_height"]
                if tx_cc["in_longest"]
                else [blk_hash[:10], "...", blk_hash[-4:]],
                "</a> (",
                format_time(tx_cc["block_nTime"]),
                ")<br />\n",
            ]

        if chain is None:
            self.log.warning("Assuming default chain for Transaction %s", tx["hash"])
        ...
        body += [
            ...
            "Total in: ",
            format_satoshis(tx["value_in"], chain),
            "<br />\n",
            "Number of outputs: ",
            len(tx["out"]),
            ' (<a href="#outputs">Jump to outputs</a>)<br />\n',
            "Total out: ",
            format_satoshis(tx["value_out"], chain),
            "<br />\n",
            "Size: ",
            tx["size"],
            " bytes<br />\n",
        ...
        if self.store.keep_scriptsig:
            body += ["<th>ScriptSig</th>"]
        body += ["</tr>\n"]
        for txin in tx["in"]:
            row_to_html(txin, "i", "o", "Generation" if is_coinbase else "Unknown")
        body += [
            "</table>\n",
    ...
    def handle_rawtx(self, page):
        self.do_raw(page, self.do_rawtx)

    def do_rawtx(self, page, chain):  # pylint: disable=unused-argument
        tx_hash = wsgiref.util.shift_path_info(page["env"])
        if (
            tx_hash in (None, "")
            or page["env"]["PATH_INFO"] != ""
            or not is_hash_prefix(tx_hash)
        ):
            return "ERROR: Not in correct format"  # BBE compatible

        tx = self.store.export_tx(tx_hash=tx_hash.lower())
        if tx is None:
            return "ERROR: Transaction does not exist."  # BBE compatible
        return json.dumps(tx, sort_keys=True, indent=2)

    def handle_address(self, page):
        ...
        def format_amounts(amounts, link):
            ...
            for chain in _chains:
                if ret:
                    ret += [", "]
                ret += [
                    format_satoshis(amounts[chain.id], chain),
                    " ",
                    escape(chain.code3),
                ]
                if link:
                    ...
                    if (
                        page["chain"] is not None
                        and version == page["chain"].script_addr_vers
                    ):
        ...
        if self.shortlink_type == "firstbits":
            link = self.store.get_firstbits(
                address_version=version,
                db_pubkey_hash=self.store.binin(binaddr),
            ...
            link = address[0 : self.shortlink_type]
        body += self.short_link(page, "a/" + link)
        ...
        for elt in txpoints:
            ...
            body += [
                '<tr class="',
                _type,
                '"><td class="tx"><a href="../tx/',
                elt["tx_hash"],
                "#",
                "i" if elt["is_out"] else "o",
                elt["pos"],
                '">',
                elt["tx_hash"][:10],
                "...</a>",
                '</td><td class="block"><a href="../block/',
                elt["blk_hash"],
                '">',
                elt["height"],
                '</a></td><td class="time">',
                format_time(elt["nTime"]),
                '</td><td class="amount">',
            ]
    ...
    def handle_search(self, page):
        ...
        if HEIGHT_RE.match(q):
            found += self.search_number(int(q))
        if util.possible_address(q):
            found += self.search_address(q)
        elif ADDR_PREFIX_RE.match(q):
            found += self.search_address_prefix(q)
        if is_hash_prefix(q):
            found += self.search_hash_prefix(q)
        found += self.search_general(q)
        self.show_search_results(page, found)

    def show_search_results(self, page, found):
        if not found:
            page["body"] = ["<p>No results found.</p>\n", self.search_form(page)]
            return
    ...
    def search_number(self, n):
        def process(row):
            ...
            hexhash = self.store.hashout_hex(dbhash)
            if in_longest == 1:
                name = str(n)
            else:
                name = hexhash
    ...
    def search_hash_prefix(self, q, types=("tx", "block", "pubkey")):
        q = q.lower()
        ret = []
        for t in types:
            ...
            def process(row):
                if t == "tx":
                    ...
                    return self._found_address(
                        util.hash_to_address("\0", self.store.binout(row[0]))
                    )
                _hash = self.store.hashout_hex(row[0])
                return {
                    "name": name + " " + _hash,
                    "uri": t + "/" + _hash,
                }
            ...
            if t == "pubkey":
                if len(q) > 40:
                    continue
                low = self.store.binin_hex(q + "0" * (40 - len(q)))
                high = self.store.binin_hex(q + "f" * (40 - len(q)))
            else:
                low = self.store.hashin_hex(q + "0" * (64 - len(q)))
                high = self.store.hashin_hex(q + "f" * (64 - len(q)))
    ...
    def _found_address(self, address):
    ...
    def search_address(self, address):
        try:
            ...
            return self.search_address_prefix(address)
        return [self._found_address(address)]

    def search_address_prefix(self, ap):
        ret = []
        ones = 0
        for count_iter in ap:
            if count_iter != "1":
                break
            ones += 1
        ...
        def process(row):
            _hash = self.store.binout(row[0])
            address = util.hash_to_address(vl, _hash)
            if address.startswith(ap):
                v = vl
            else:
                if vh != vl:
                    address = util.hash_to_address(vh, _hash)
                    if not address.startswith(ap):
                        return None
                    v = vh
            if self.is_address_version(v):
                return self._found_address(address)
        ...
        while l >= minlen:
            ...
            if ones:
                if not all_ones and util.hash_to_address("\0", hh)[ones:][:1] == "1":
                    break
            elif vh == "\0":
                break
            ...
            if hl <= hh:
                ...
                neg = " NOT"
                hl, hh = hh, hl
            bl = self.store.binin(hl)
            bh = self.store.binin(hh)
            ret += filter(
                None,
                map(
                    process,
    ...
    def search_general(self, q):
        ...
        def process(row):
            (name, code3) = row
            return {"name": name + " (" + code3 + ")", "uri": "chain/" + str(name)}
    ...
    def handle_t(self, page):
        self.show_search_results(
            page,
            self.search_hash_prefix(
                b58hex(wsgiref.util.shift_path_info(page["env"])), ("tx",)
            ),
        )

    def handle_b(self, page):
        if page.get("chain") is not None:
            ...
            self._show_block(
                page, page["dotdot"] + "block/", chain, block_number=height
            )
            return
        ...
        self.show_search_results(
            page,
            self.search_hash_prefix(
                shortlink_block(wsgiref.util.shift_path_info(page["env"])), ("block",)
            ),
        )

    def handle_a(self, page):
        arg = wsgiref.util.shift_path_info(page["env"])
        if self.shortlink_type == "firstbits":
            addrs = map(
                self._found_address,
                self.store.firstbits_to_addresses(
                    arg.lower(), chain_id=page["chain"] and page["chain"].id
                ),
            )
        else:
            addrs = self.search_address_prefix(arg)
        self.show_search_results(page, addrs)

    def handle_unspent(self, page):
        self.do_raw(page, self.do_unspent)

    def do_unspent(self, page, chain):
        ...
        if len(addrs) < 1 or len(addrs) > MAX_UNSPENT_ADDRESSES:
            return "Number of addresses must be between 1 and " + str(
                MAX_UNSPENT_ADDRESSES
            )
        ...
        for address in addrs:
            try:
                hashes.append(self.store.binin(base58.bc_address_to_hash_160(address)))
        ...
        max_rows = self.address_history_rows_max
        if max_rows >= 0:
            bind += [max_rows + 1]

        spent = set()
        for txout_id, spent_chain_id in self.store.selectall(
            """
        ...
        self.log.debug("spent: %s", spent)

        received_rows = self.store.selectall(
            """
            SELECT
                txout.txout_id,
                cc.chain_id,
        ...
        for row in rows:
            ...
            tx_hash = self.store.hashout_hex(tx_hash)
            out_pos = None if out_pos is None else int(out_pos)
            script = self.store.binout_hex(script)
            ...
            out.append(
                {
                    ...
                    "script": script,
                    "value": value,
                    "value_hex": None if value is None else f"{hex(value)}",
                    "block_number": height,
                }
            )
    ...
    def do_raw(self, page, func):
        page["content_type"] = "text/plain"
        page["template"] = "%(body)s"
        page["body"] = func(page, page["chain"])

    def handle_q(self, page):
        cmd = wsgiref.util.shift_path_info(page["env"])
        if cmd is None:
            return self.q(page)

        func = getattr(self, "q_" + cmd, None)
        if func is None:
            raise PageNotFound()

        self.do_raw(page, func)
    ...
    def get_max_block_height(self, chain):
        # "getblockcount" traditionally returns max(block_height),
        # which is one less than the actual block count.
        return self.store.get_block_number(chain.id)

    def q_getblockcount(self, page, chain):
        ...
        return self.get_max_block_height(chain)

    def q_getdifficulty(self, page, chain):
        ...
        target = self.store.get_target(chain.id)
        return "" if target is None else util.target_to_difficulty(target)

    def q_translate_address(self, page, chain):
        ...
        version, _hash = util.decode_check_address(addr)
        if _hash is None:
            return addr + " (INVALID ADDRESS)"
        return util.hash_to_address(chain.address_version, _hash)

    def q_decode_address(self, page, chain):
        """shows the version prefix and hash encoded in an address."""
        addr = wsgiref.util.shift_path_info(page["env"])
        if addr is None:
            return (
                "Shows ADDRESS's version byte(s) and public key hash"
                ' as hex strings separated by colon (":").\n'
                "/q/decode_address/ADDRESS\n"
            )
        # XXX error check?
        version, _hash = util.decode_address(addr)
        ret = version.encode("hex") + ":" + _hash.encode("hex")
        if util.hash_to_address(version, _hash) != addr:
            ret = "INVALID(" + ret + ")"
        return ret

    def q_addresstohash(self, page, chain):
        ...
        version, _hash = util.decode_address(addr)
        return _hash.encode("hex").upper()

    def q_hashtoaddress(self, page, chain):
        ...
        if arg2 is not None:
            ...
            version, _hash = arg2, arg1

        elif arg1.find(":") >= 0:
            # VERSION:HASH as returned by /q/decode_address.
            version, _hash = arg1.split(":", 1)

        elif chain:
            version, _hash = chain.address_version.encode("hex"), arg1

        else:
            # Default: Bitcoin address starting with "1".
            version, _hash = "00", arg1

        try:
            _hash = _hash.decode("hex")
            version = version.decode("hex")
        except Exception:
            return "ERROR: Arguments must be hexadecimal strings of even length"
        return util.hash_to_address(version, _hash)

    def q_hashpubkey(self, page, chain):
    ...
    def q_checkaddress(self, page, chain):
    ...
    def q_nethash(self, page, chain):
        ...
        if interval < 0 or start < 0 or (stop is not None and stop < 0):
            count = self.get_max_block_height(chain)
            if start < 0:
                start += count
        ...
        rows = self.store.selectall(
            ...
            if stop is None
            else (interval, start, chain.id, stop_ix),
        )
        if fmt == "csv":
            ret = NETHASH_HEADER
            ...
            page["template_vars"]["block_time"] = 600
            # XXX BTC-specific
            ret = ""
        ...
        for row in rows:
            height, n_time, chain_work, n_bits = row
            n_time = float(n_time)
            n_bits = int(n_bits)
            target = util.calculate_target(n_bits)
            difficulty = util.target_to_difficulty(target)
            work = util.target_to_work(target)
            chain_work = self.store.binout_int(chain_work) - work

            if row is not rows[0] or fmt == "svg":
                height = int(height)
                interval_work = chain_work - prev_chain_work
                avg_target = util.work_to_target(interval_work / float(interval))
                # if avg_target == target - 1:
                #    avg_target = target
                interval_seconds = n_time - prev_nTime
                ...
                if fmt == "csv":
                    ...
                    ret += '<abe:nethash t="%d" d="%d"' ' w="%d"/>\n' % (
                        n_time,
                        work,
                        interval_work,
                    )

            prev_nTime, prev_chain_work = n_time, chain_work
    ...
    def q_getreceivedbyaddress(self, page, chain):
        ...
        version, _hash = util.decode_address(addr)
        return format_satoshis(self.store.get_received(chain.id, _hash), chain)

    def q_getsentbyaddress(self, page, chain):
        """shows the amount ever sent from a given address."""
        addr = wsgiref.util.shift_path_info(page["env"])
        if chain is None or addr is None:
            return (
                "returns amount of money sent from given address\n"
        ...
        version, _hash = util.decode_address(addr)
        return format_satoshis(self.store.get_sent(chain.id, _hash), chain)

    def q_addressbalance(self, page, chain):
        ...
        version, _hash = util.decode_address(addr)
        total = self.store.get_balance(chain.id, _hash)

        return (
            "ERROR: please try again"
            if total is None
            else format_satoshis(total, chain)
        )

    def q_fb(self, page, chain):
        """returns an address's firstbits."""

        if not self.store.use_firstbits:
            raise PageNotFound()

        addr = wsgiref.util.shift_path_info(page["env"])
        if addr is None:
            return (
                "Shows ADDRESS's firstbits:"
        ...
        ret = self.store.get_firstbits(
            address_version=version,
            db_pubkey_hash=self.store.binin(dbhash),
    ...
    def q_addr(self, page, chain):
        ...
        if fb is None:
            return (
                "Shows the address identified by FIRSTBITS:"
        ...
        return "\n".join(
            self.store.firstbits_to_addresses(fb, chain_id=(chain and chain.id))
        )

    def handle_download(self, page):
        name = self.args.download_name
        if name is None:
    ...
    def serve_static(self, path, start_response):
        slen = len(self.static_path)
        if path[:slen] != self.static_path:
            raise PageNotFound()
        path = path[slen:]
        try:
            ...
            found = open(self.htdocs + "/" + path, "rb")

            _type, enc = mimetypes.guess_type(path)
            # XXX Should do something with enc if not None.
            # XXX Should set Content-length.
            start_response("200 OK", [("Content-type", _type or "text/plain")])
            return found
        except IOError:
            raise PageNotFound()

    # Change this if you want empty or multi-byte address versions.
    def is_address_version(self, v):
        return len(v) == 1

    def short_link(self, page, link):
        base = self.base_url
        if base is None:
            env = page["env"].copy()
        ...
        return [
            '<p class="shortlink">Short Link: <a href="',
            page["dotdot"],
            link,
            '">',
            full,
...
def format_time(n_time):
    ...
    return time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(n_time)))
...
def format_satoshis(satoshis, chain):
    ...
    frac = satoshis % coin
    return str(integer) + ("." + (("0" * decimals) + str(frac))[-decimals:]).rstrip(
        "0"
    ).rstrip(".")
...
def hash_to_address_link(version, _hash, dotdot, truncate_to=None, text=None):
    if _hash == data_store.NULL_PUBKEY_HASH:
        return "Destroyed"
    if _hash is None:
        return "UNKNOWN"
    addr = util.hash_to_address(version, _hash)
...
def redirect(page):
    ...
    page["start_response"](
        "301 Moved Permanently", [("Location", uri), ("Content-Type", "text/html")]
    )
    return (
...
def create_conf():
    ...
    conf.update(data_store.CONFIG_DEFAULTS)
    return conf
...
See abe.conf for commented examples."""
        ...
        print("Schema version", data_store.SCHEMA_VERSION)
        return 0
